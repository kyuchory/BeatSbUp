...
router.get("/checkDate", function (req, res) {
  ...
  router.get("/addSch", function (req, res) {
    connection.query(`INSERT INTO schedule (id, sight_id, start, end, date, offset) VALUES ('${req.query.id}', '${req.query.sight}', '${req.query.start}', '${req.query.end}', '${req.query.date}','${req.query.offset}');`, function (error, results, fields) {
      if (error) {
        console.log(error);
      } else {
        console.log(results);
  ...
  router.get("/getSchedule", function (req, res) {
    connection.query(`SELECT * FROM schedule WHERE id='${req.query.id}' AND offset='${req.query.offset}';`, function (error, results, fields) {
      if (error) {
        console.log(error);
      } else {
        res.json(results);
