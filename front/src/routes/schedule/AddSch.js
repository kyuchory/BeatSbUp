...
function AddSch() {
    ...
    return (
        ...
        index === selected2 ? (
            <button onClick={() => select2(index)} className={styles.gatheringEles}>{index}일차</button>
        ) : (
            <button onClick={() => select2(index)} className={styles.gatheringEle}>{index}일차</button>
        )
      ))}
      <br /></div></div>
      <div>시작시간 : <TimePicker onChange={onChange} value={value} /></div>
      <div>종료시간 : <TimePicker onChange={onChange2} value={value2} /></div>
      <div>장소 : <input onChange={handleChange} value={sight}></input><br /></div>
